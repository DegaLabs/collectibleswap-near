...
use crate::curves::{BuyInfo, SellInfo, U256, WAD};
...
impl fmt::Debug for DepositedToken {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("DepositedToken")
            .field("x", &self.depositor)
            .field("y", &self.min_price)
            .finish()
    }
}
...
pub struct Pair {
    ...
    pub unrealized_fees: u128,
}

impl fmt::Debug for Pair {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("Pair")
            .field("y", &self.nft_token)
            .field("y", &self.spot_price)
            .field("y", &self.delta)
            .field("y", &self.fee)
            .field("y", &self.owner)
            .field("y", &self.asset_recipient)
            .field("y", &self.near_balance)
            .field("y", &self.token_ids_in_pools)
            .field("y", &self.released_time)
            .field("y", &self.pool_id)
            .finish()
    }
}
...
impl Pair {
    ...
    pub fn deposit_and_mint_lp(
        ...
        if self.pool_type == PoolType::Trade {
            require!(
                token_ids.len() as u128 * self.spot_price <= near_balance.clone(),
        ...
        if self.lp_supply != 0 && !self.token_ids_in_pools.is_empty() {
            lp_amount =
                self.lp_supply * token_ids.len() as u128 / self.token_ids_in_pools.len() as u128;
        }
        self.mint_lp(&receiver_id, lp_amount);
    }

    fn internal_add_token_ids(&mut self, depositor: AccountId, token_ids: &Vec<TokenId>) {
        for token_id in token_ids {
            self.token_ids_in_pools.insert(
                token_id,
                &DepositedToken {
    ...
    pub fn withdraw_near(&mut self, near_amount: &Balance) -> Balance {
        require!(
            self.pool_type != PoolType::Trade,
            "not allowed to withdraw near directly from trading pool, need to burn lp token"
        );
        self.assert_owner();
        self.assert_release();
    ...
    pub fn withdraw_nfts(&mut self, token_ids: &Vec<TokenId>) {
        require!(
            self.pool_type != PoolType::Trade,
            "not allowed to withdraw nfts directly from trading pool, need to burn lp token"
        );
        self.assert_owner();
        self.assert_release();
        for token_id in token_ids {
    ...
    pub fn swap_near_for_any_nfts(
        ...
        if u128::MAX - input_amount < self.accumulated_volume {
            self.accumulated_volume_times += 1;
            self.accumulated_volume = self.accumulated_volume - (u128::MAX - input_amount);
        } else {
            self.accumulated_volume = self.accumulated_volume + input_amount;
        }
    ...
    fn calculate_buy_info_and_update_pool(
        ...
        let (buy_info, unrealized_fee) = self.get_buy_info(
            current_spot_price,
            current_delta,
            num_nfts,
            self.fee,
        ...
        self.accumulated_fees += buy_info.trade_fee;
        self.unrealized_fees = unrealized_fee;
    ...
    pub(crate) fn get_buy_info(
        ...
        let mut ret = BuyInfo {
            ...
            protocol_fee: U256::from(0 as u64),
            trade_fee: 0
        };
        let mut unrealized_fees = self.unrealized_fees;
        ...
        while i < num_items {
            ...
            ret.trade_fee += buy_info.trade_fee;
            ret.protocol_fee += buy_info.protocol_fee;
            ret.input_value += buy_info.input_value;

            let nft_count_after_swap = (self.token_ids_in_pools.len() - (i + 1)) as u128;
            let mut price_increase = 0;
            if nft_count_after_swap > 0 {
                price_increase = (buy_info.trade_fee + unrealized_fees) / nft_count_after_swap;
            }
            unrealized_fees = buy_info.trade_fee + unrealized_fees - price_increase * nft_count_after_swap;
            ret.new_spot_price += price_increase;
            i += 1;
        }
        return (ret, unrealized_fees)
    }
    ...
    pub(crate) fn get_sell_info(
        ...
        while i < num_items {
            ...
            let nft_count_after_swap = (self.token_ids_in_pools.len() + i + 1) as u128;
            let price_increase = (sell_info.trade_fee + unrealized_fees) / nft_count_after_swap;
            unrealized_fees = sell_info.trade_fee + unrealized_fees - price_increase * nft_count_after_swap;
            ret.new_spot_price += price_increase;
            i += 1;
        }
    ...
    fn calculate_sell_info_and_update_pool(
        ...
        let (sell_info, unrealized_fee) = self.get_sell_info(
            current_spot_price,
            current_delta,
            num_nfts,
            self.fee,
        ...
        self.accumulated_fees += sell_info.trade_fee;
        self.unrealized_fees = unrealized_fee;
    ...
    pub fn lp_transfer(&mut self, sender_id: &AccountId, receiver_id: &AccountId, amount: u128) {
        let balance = self
            .lp_balances
            .get(&sender_id)
            .expect("sender account not registered");
        ...
        self.lp_balances
            .insert(&receiver_id, &(balance_out + amount));
    }
    ...
    pub fn burn_lp(
        ...
        log!("{:?}", self.token_ids_in_pools.len());
        let withdrawable_near = U256::from(self.token_ids_in_pools.len())
            * U256::from(self.spot_price)
            * U256::from(lp)
            / U256::from(self.lp_supply);
        log!("{:?}", withdrawable_near);
        ...
        if num_nfts_to_withdraw * self.lp_supply != lp * self.token_ids_in_pools.len() as u128 {
            ...
            let (buy_info, _) = self.get_buy_info(
                self.spot_price,
                self.delta,
                1,
                self.fee,
            ...
            value_in_fraction_nft =
                (num_nfts_to_withdraw - 1) * self.spot_price + 1 * buy_info.new_spot_price;
            require!(
                value_in_fraction_nft >= withdrawable_near.clone(),
                "internal error in handling liquidity"
            );
            value_in_fraction_nft -= withdrawable_near.clone();
        }
        ...
        let protocol_fee =
            U256::from(withdrawable_near) * U256::from(protocol_fee_multiplier) / WAD;
