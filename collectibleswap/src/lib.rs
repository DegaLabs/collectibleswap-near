...
mod tests {
    ...
    fn asset_id() -> AssetId {
        AccountId::new_unchecked("asset_id.near".to_string())
    }
     
    fn token_ids() -> Vec<TokenId> {
        let mut token_ids = Vec::<TokenId>::new();
        token_ids.push("token_id1.near".to_string());
        token_ids.push("token_id2.near".to_string());
        token_ids
    }
    ...
    #[test]
    fn test_set_protocol_fee_receiver() {
        let mut context = get_context(governance_id());
        testing_env!(context.build());
        ...
        contract.set_protocol_fee_receiver(governance_id());
        //testing_env!(context.is_view(true).build());
        assert_eq!(contract.protocol_fee_receiver_id, governance_id());
    }
    ...
    fn test_create_pair() {
        ...
        testing_env!(context
            ...
            .predecessor_account_id(governance_id())
            .build());

        contract.storage_deposit(Some(governance_id()), Some(true));

        contract.nft_on_transfer(governance_id(), governance_id(), token_id(), "a".to_string());
        ...
        contract.create_pair(
            2u8, // pool_type: u8
            0u8, // bonding_curve: u8
            governance_id(), // asset_id: AssetId
            U128(10u128), // spot_price: U128
            U128(10u128), // delta: U128
            U128(10u128), // fee: U128
            None, // asset_recipient: Option<AccountId>
            token_ids(), // initial_token_ids: Vec<TokenId>,
            0 // locked_til: u64,
        );
        
        assert_ne!(contract.pools.len(), 0);
        assert_ne!(contract.internal_get_account_or_revert(&governance_id()).near_balance, 0);
        assert_ne!(contract.pools[0].lp_supply, 0);
        assert_ne!(contract.pools[0].lp_balances.get(&governance_id()).unwrap(), 0);
    ...
    fn test_add_liquidity() {
        ...
        contract.storage_deposit(governance_id().into(), None);
        ...
        contract.add_liquidity(0, token_ids());

        assert_ne!(contract.pools.len(), 0);
        assert_ne!(contract.pools[0].near_balance, 0);
        assert_ne!(contract.pools[0].lp_supply, 0);
        assert_ne!(contract.pools[0].lp_balances.get(&governance_id()).unwrap(), 0);
    ...
    fn test_remove_liquidity() {
        ...
        contract.remove_liquidity(0, U128(1_000_000_000_000_000_000_000_000));

        assert_ne!(contract.protocol_fee_credit, 0);
        assert_eq!(contract.pools[0].lp_supply, 0);
        assert_eq!(contract.pools[0].lp_balances.get(&governance_id()).unwrap(), 0);
    ...
    fn test_withdraw_near() {
        ...
        contract.create_pair(
            ...
            U128(10u128), // delta: U128
            U128(0u128), // fee: U128
            Some(governance_id()), // asset_recipient: Option<AccountId>
            token_ids(), // initial_token_ids: Vec<TokenId>,
            0 // locked_til: u64,
        );

        contract.withdraw_near(0, U128(1_000_000_000_000_000_000_000_000));

        assert_eq!(contract.pools[0].near_balance, 0);
    ...
    fn test_withdraw_nfts() {
        ...
        contract.withdraw_nfts(0, token_ids());

        assert_eq!(contract.pools[0].token_ids_in_pools.len(), 0);
        //assert_ne!(contract.internal_get_account_or_revert(&governance_id()).near_balance, 0);

    }

    #[test]
    fn test_withdraw_nfts_from_deposit() {
        let mut context = get_context(governance_id());
        testing_env!(context.build());
        ...
        contract.withdraw_nfts_from_deposit(governance_id(), token_ids());

        assert_ne!(contract.internal_get_account_or_revert(&governance_id()).assets.len(), 0);
